...
class FGES:
    ...
    def fes(self):
        ...
        while len(self.sorted_arrows) > 0:
            ...
            self.printm("Edge set before reapplying orientation: " + str(self.graph.edges()))
            visited_nodes = self.reapply_orientation(x, y, None)  # Orient edges appropriately following insertion
            self.printm("Edge set after reapplying orientation: " + str(self.graph.edges()))
            to_process = set({})
    ...
    def bes(self): # TODO: FLAG
        ...
        while len(self.sorted_arrows) > 0:
            ...
            self.printm("BES: Removed arrow " + str(x) + " -> " + str(y) + " with bump -" + str(bump))
            if self.verbose:
                self.printm("BES: Removed arrow " + str(x) + " -> " + str(y) + " with bump -" + str(bump)) # TODO: FLAG
    ...
    def calculate_arrows_backward(self, a, b):
        ...
        for i in range(_depth + 1):
            ...
            for choice in choices:
                ...
                if bump > 0:
                    if self.verbose:
                        self.printm("Evaluated removal of an arrow " + str(
                            a) + " -> " + str(b) + " with bump: " + str(bump))
                    self.add_arrow(a, b, na_y_x, h, bump)

    def delete_eval(self, x, y, diff, na_y_x): #TODO: FLAG
    ...
    def reevaluate_forward(self, to_process, arrow):
        self.printm("Re-evaluate forward with " + str(to_process) + " " + str(arrow))
        for node in to_process:
            if self.mode == "heuristic":
                nzero_effect_nodes = self.effect_edges_graph.get(node)
                print("Re-evaluate forward. Currently on node: " + str(node))
                self.printm("nzero-effect-nodes: " + str(nzero_effect_nodes))
    ...
    def add_required_edges(self):
        ...
        for edge in self.knowledge.required_edges:
            ...
            if not edge[0] in graph_util.get_ancestors(self.graph, edge[1]):
                ...
                if self.verbose:
                    self.printm(f"Adding edge from knowledge: {edge[0]} -> {edge[1]}")
    ...
    def initialize_forward_edges_from_empty_graph(self):
        ...
        for i in range(len(self.variables)):
            ...
            for j in range(i + 1, len(self.variables)):
                ...
                if self.verbose:
                    self.printm("Evaluated starting arrow " + str(self.variables[j]) + " -> " + str(
        ...
        if self.verbose:
            self.printm("Initialized forward edges from empty graph")
    ...
    def insert_eval(self, x, y, T, na_y_x): # TODO: INFO / FLAG
    ...
    def insert(self, x, y, T, bump): # TODO: Info
        ...
        """
        if self.verbose:
            self.printm("Doing an actual insertion with " + str(x) + " -> " + str(
    ...
    def delete(self, x, y, H): # TODO: FLAG
        ...
        self.removed_edges.add((x, y)) # TODO: INFO
    ...
    def add_arrow(self, a, b, na_y_x, h_or_t, bump):
        ...
        self.printm("Added arrow: " + str(a) + " ->  " + str(b) + " with bump " + \
    ...
    def clear_arrow(self, a, b):
        ...
        if lookup_arrows is not None:
            for arrow in lookup_arrows:
                self.printm("Removing " + str(arrow) + " from sorted_arrows")
    ...
    def calculate_arrows_forward(self, a, b):
        self.printm("Calculate Arrows Forward: " + str(a) + " " + str(b))
        if b not in self.effect_edges_graph[a] and self.mode == "heuristic":
            self.printm("Returning early...")
            return

        if self.knowledge is not None and self.knowledge.is_forbidden(a, b):
            return

        self.printm("Get neighbors for " + str(b) + " returns " + str(graph_util.neighbors(self.graph, b)))
        ...
        self.printm("tneighbors for " + str(a) + ", " + str(b) + " returns " + str(t_neighbors))
        ...
        def outer_loop():
            ...
            for i in range(len_T + 1):
                ...
                self.printm("All choices: ", list(choices2), " TNeighbors: ", t_neighbors)
                for choice in choices:
                    T = frozenset([t_neighbors[k] for k in choice])
                    self.printm("Choice:", T)
                    union = set(na_y_x)
                    union.update(T)
                    ...
                    self.printm("Evaluated arrow " + str(a) + " -> " + str(b) + " with T: " + str(T) + " and bump: " + str(bump));
